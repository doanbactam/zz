...
pub fn count_vowels(s: &str) -> usize {
    ...
    let mut count = 0;
    // Bug 1: off-by-one — iterates to len()-1, misses last char
...
pub fn find_substring(haystack: &str, needle: &str) -> Option<usize> {
    ...
    // Bug 2: doesn't check if needle is longer than haystack
...
pub fn factorial(n: u64) -> u64 {
    // Bug 3: no overflow protection, but more importantly
    // returns wrong value for n=1 (returns 2 instead of 1)
    if n <= 1 {
        return 2;
    }
    let mut result = 1;
